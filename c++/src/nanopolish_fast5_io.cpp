...
std::string fast5_get_sequence(fast5_file& fh, const std::string& read_id)
{
    herr_t status;
    std::string read_group = fast5_get_analyses_read_group(fh, read_id);
    std::string seq_path = read_group + "Fastq";
    hid_t dset = H5Dopen(fh.hdf5_file, seq_path.c_str(), H5P_DEFAULT);
    hid_t filetype = H5Dget_type (dset);
    hid_t native_type = h5tools_get_native_type(filetype);
    const size_t type_size {std::max(H5Tget_size(filetype), H5Tget_size(native_type))};
    char* fqptr = (char*)malloc(type_size);
    status = H5Dread (dset, native_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, fqptr);
    std::string fastq(fqptr);
    std::vector<std::string> fastq_lines;
    boost::split(fastq_lines, fastq, boost::is_any_of("\n"));
    return fastq_lines.at(1);
...
//
std::string fast5_get_analyses_read_group(fast5_file& fh, const std::string& read_id)
{
    if(fh.is_multi_fast5) {
        return "/read_" + read_id + "/Analyses/Basecall_1D_000/BaseCalled_template/";
    } else {
